...
func CreateTCPSyslogServer(port string) (syslogserver.LogPartsChannel, *syslogserver.Server) {
	...
	if err := server.ListenTCP("0.0.0.0:" + port); err != nil {
		log.Fatal(err.Error())
	}
...
func TestSyslogTcpOutput(t *testing.T) {
	...
	Convey("Ensure we can send syslog packets", t, func() {
		channel, server := CreateTCPSyslogServer("8512")
		go server.Wait()

		Convey("Ensure we can start the tcp syslog end point on port 8512.", func() {
